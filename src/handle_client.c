...
int handle_client_interaction(int client_sockfd, FILE *server_log, char *client_ip, client_session_t *session)
{
   ...
   while (1)
    {
        ...
        for (ssize_t i = 0; i < n; i++)
        {
            ...
            if (c == '\n')
            {
                ...
                fprintf(server_log, "[%s] [INFO] (RECV) IP='%s' ID='%d' DATA='%s'\n", get_timestamp(), client_ip, session->id ,line_buffer);
                ...
                {
                    fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' BYTES=%zu HELP OUTPUT SENT\n", get_timestamp(), client_ip, session->id ,sizeof(HELP_MESSAGE));
                ...
                {
                    fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' BYTES=%zu VERSION OUTPUT SENT\n", get_timestamp(), client_ip, session->id ,sizeof(TINYSHELL_VERSION));
                ...
                {
                    fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' BYTES=%zu ECHO OUTPUT SENT\n", get_timestamp(), client_ip, session->id, sizeof(line_buffer));
                ...
                else if (parse_status == DATE_OK)
                {
                    ...
                    fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' BYTES=%zu DATE OUTPUT SENT\n", get_timestamp(), client_ip, session->id ,sizeof(date));
                ...
                {
                    fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' BYTES=%zu ABOUT OUTPUT SENT\n", get_timestamp(), client_ip, session->id ,sizeof(ABOUT_MESSAGE));
                ...
                {
                    fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' BYTES=%zu USER EXITED SUCESSFULLY\n", get_timestamp(), client_ip, session->id ,sizeof(QUIT_MESSAGE));
                ...
                {
                    fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' BYTES=%zu CLEAR COMMAND SENT\n", get_timestamp(), client_ip, session->id ,sizeof("\033[2J\033[H"));
                ...
                {
                    fprintf(server_log, "[%s] [WARN] (SEND) IP='%s' ID='%d' BYTES=%zu PERMISSION DENIED, FOR AUTHENTICATED USERS ONLY\n", get_timestamp(), client_ip, session->id ,sizeof(FATAL_ERROR_AUTHENTICATED_USERS_ONLY));
                ...
                else if (parse_status == LOGIN_OK && session->s_state == SESSION_WAITING_LOGIN) // comando LOGIN com estado de conexão correto
                {
                    ...
                    if (auth_status == true)
                    {
                        fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' USER='%s' BYTES=%zu AUTHENTICATION SUCCESSFULLY\n", get_timestamp(), client_ip, session->id ,session->username, sizeof(INFO_AUTHENTICATION_SUCCEEDED));
                    ...
                    {
                        fprintf(server_log, "[%s] [WARN] (SEND) IP='%s' ID='%d' BYTES=%zu AUTHENTICATION FAILED\n", get_timestamp(), client_ip, session->id ,sizeof(FATAL_ERROR_AUTHENTICATION_FAILED));
                ...
                {
                    fprintf(server_log, "[%s] [WARN] (SEND) IP='%s' ID='%d' USER='%s' BYTES=%zu AUTHENTICATED ALREADY\n", get_timestamp(), client_ip, session->id ,session->username, sizeof(FATAL_ERROR_USER_ALREADY_AUTHENTICATED));
                ...
                else if (parse_status == REGISTER_OK && strcmp(session->username, admin_name) == 0) // comando REGISTER com estado de conexão correto
                {
                    ...
                    fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' BYTES=%zu USER REGISTERED SUCCESSFULLY\n", get_timestamp(), client_ip, session->id ,sizeof(INFO_USER_INSERTED_DATABASE_SUCCESSFULLY));
                ...
                {
                    fprintf(server_log, "[%s] [WARN] (SEND) IP='%s' ID='%d' BYTES=%zu PERMISSION DENIED, FOR ADMINISTRADOR ONLY\n", get_timestamp(), client_ip, session->id ,sizeof(FATAL_ERROR_ADMINS_ONLY));
                ...
                {
                    fprintf(server_log, "[%s] [INFO] (SEND) IP='%s' ID='%d' BYTES=%zu USER LOGOUT SUCCESSFULLY\n", get_timestamp(), client_ip, session->id ,sizeof(INFO_USER_LOGOUT_SUCCESSFULL));
                ...
                {
                    fprintf(server_log, "[%s] [WARN] (SEND) IP='%s' ID='%d' BYTES=%zu USER LOGOUT FAILED, NOT AUTHENTICATED\n", get_timestamp(), client_ip, session->id ,sizeof(FATAL_ERROR_AUTHENTICATED_USERS_ONLY));
                    send(client_sockfd, FATAL_ERROR_AUTHENTICATED_USERS_ONLY, strlen(FATAL_ERROR_AUTHENTICATED_USERS_ONLY), 0);
                    send_prompt(session);
                }
                ...
                {
                    fprintf(server_log, "[%s] [WARN] (SEND) IP='%s' ID='%d' BYTES=%zu COMMAND NOT FOUND\n", get_timestamp(), client_ip, session->id ,sizeof(FATAL_ERROR_COMMAND_NOT_FOUND));
